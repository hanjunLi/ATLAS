...
ZpKaratsubaElement::ZpKaratsubaElement() { elem = 0; }
...
operator+=(const ZpKaratsubaElement &f2) {
...
ZpKaratsubaElement &ZpKaratsubaElement::
operator=(const ZpKaratsubaElement &other) // copy assignment
{
 ...
 * ax + by = gcd(a,b)
 */
void gcd(unsigned long a, unsigned long b, unsigned long &gcd, unsigned long &x,
         unsigned long &y) {
 ...
 * If there is more than one (i.e. when gcd(B,m)>1) - returns the smallest such
 * integer
 */
ZpKaratsubaElement ZpKaratsubaElement::operator/(const ZpKaratsubaElement &f2) {

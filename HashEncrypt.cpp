...
HashEncrypt::HashEncrypt(const unsigned char *key, const unsigned char *iv,
  ...
  EVP_EncryptInit_ex(&_ctx, EVP_aes_128_gcm(), NULL, NULL,
                     NULL); // TODO check return value == 1
...
void HashEncrypt::hashUpdate(unsigned char *in, int inSizeBytes) {
  ...
  EVP_EncryptUpdate(&_ctx, NULL, &_unusedOutl, in,
                    inSizeBytes); // TODO check return value == 1
}
...
void HashEncrypt::hashFinal(unsigned char *out, unsigned int *outSizeBytes) {
  ...
  EVP_EncryptFinal_ex(&_ctx, NULL, &_unusedOutl); // TODO check return value ==
                                                  // 1
